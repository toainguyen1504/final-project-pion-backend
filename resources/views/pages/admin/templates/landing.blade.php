<section class="template-landing p-5 bg-gradient text-light text-center">
    ...
    <p class="lead mb-3">{{ optional($posts->category)->name }}</p>

    @if ($posts->images()->where('type', 'post_thumbnail')->first())
        <img src="{{ $posts->images()->where('type', 'post_thumbnail')->first()->path }}" class="img-fluid mb-4"
            alt="Banner">
    @endif
